/** Voivodeship paths adapted from MapSVG Poland map (viewBox 0 0 612.76 577.23). */
export const POLAND_VOIV_VIEW = {
  width: 612.75696,
  height: 577.23169,
} as const;

/** MapSVG geoViewBox: west north east south */
export const POLAND_VOIV_GEO = {
  left: 14.122232,
  top: 54.834941,
  right: 24.149724,
  bottom: 49.002699,
...
export type SalonVoivodeshipGroup = {
  id: string;
  name: string;
  cities: SalonCityLink[];
};

export const polandVoivodeships: PolandVoivodeship[] = [
  {
    id: "PL-DS",
    name: "Dolno\u015bl\u0105skie",
...
export function groupSalonCitiesByVoivodeship(
  ...
  const byId = new Map(
    polandVoivodeships.map((region) => [region.id, region.name]),
  );
  ...
  for (const city of cities) {
    ...
    if (!voivId) continue;
    const existing = indexById.get(voivId);
    if (existing === undefined) {
      indexById.set(voivId, groups.length);
      groups.push({
        id: voivId,
        name: byId.get(voivId) ?? voivId,
        cities: [city],
      });
      continue;
    }
