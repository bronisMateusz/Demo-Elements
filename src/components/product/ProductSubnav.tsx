    ...
    <>
      ...
      >
        <div className="container overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <LayoutGroup id="pdp-subnav-underline">
            <ul className="mx-auto flex w-max min-w-full items-stretch justify-center gap-0 md:gap-1">
              {items.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    <a
                      href={`#${item.id}`}
                      className={cn(
                        "relative inline-flex min-h-[58px] items-center px-3 py-3 font-body text-ui leading-none text-neutral-600 no-underline transition-colors duration-fast ease-out hover:text-neutral-900 focus-visible:outline-2 focus-visible:outline-offset-[var(--spacing-focus-ring-offset)] focus-visible:outline-neutral-800 md:px-4",
                    ...
                    >
                      {isActive ? (
                        <motion.span
                          ...
                          className="pointer-events-none absolute inset-x-3 bottom-0 h-0.5 bg-neutral-900 md:inset-x-4"
