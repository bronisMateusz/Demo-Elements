...
export function productCarouselSwiperClassName(
  ...
  return cn(
    ...
    layout === "inline" || layout === "inline-bleed"
      ...
      : layout === "bleed"
        ? // Mobile: ~1.15 cards with peek; md+: ~4.2 cards + peek of the next
          // Use 100% (track width), not 100vw - avoids scrollbar overhang vs Container gutters.
          "[&_.swiper-slide]:h-auto! [&_.swiper-slide]:w-[min(18.5rem,calc(100%-3.5rem))]! md:[&_.swiper-slide]:w-[clamp(16.25rem,calc((100%-1.25rem)/4.2),25rem)]! [&_.swiper-slide]:shrink-0"
...
export function productCarouselSlideClassName(
  ...
  if (layout === "bleed") {
    return "h-auto! w-[min(18.5rem,calc(100%-3.5rem))]! shrink-0 md:w-[clamp(16.25rem,calc((100%-1.25rem)/4.2),25rem)]!";
  }
